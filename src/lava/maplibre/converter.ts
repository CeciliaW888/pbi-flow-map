...
export function fitOptions(bounds: IBound[], view: ISize): { zoom: number, center: [number, number] } {
    ...
    let width = helper.lon2x((e - w) > 180 ? (e - w) - 180 : (e - w), 20);
    ...
    return { zoom: level, center: [centerLon, centerLat] };
}

export function anchorPixel(m: maplibregl.Map, bound: IBound): IPoint {
    let level = m.getZoom();
    let { anchor, margin } = bound;
    let pix = m.project([anchor.longitude, anchor.latitude]);
